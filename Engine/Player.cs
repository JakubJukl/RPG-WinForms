...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        public bool KilledMobRequiredToEnter (Location location, int y)
        {
            if (location.KillRequiredToEnter == false)
            {
                return true;
            }
            else if (y!=0)
        ...
        {
            //if (Quests.Exists(playerQuest => playerQuest.Details.ID == quest.ID))
            //{
            //    return true;
            //}
            //return false;
        ...
        public int LevelUp(int xp, int lvl)
        {
            if (xp>=100*lvl)
            {
                lvl++;
